...
/****** Object:  StoredProcedure [dbo].[usp_ProfitandLossSummaryFact_BudgetBU_Summary]    Script Date: 24/11/2016 11:06:35 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE Procedure [dbo].[usp_ProfitandLossSummaryFact_BudgetBU_Summary] 
	@pCurrency varchar(5),
	@pFiscalPeriod int,
	@pTemplate int,
	@pLanguage int,
	@pBudgetRevision varchar(20),
	@pOrg varchar(max),
...
TemplateID,Group1Level,Group2Level,Group3Level,Group1LevelID,Group2LevelID,Group3LevelID, GroupID ,GroupDesc,
...
FROM        FactProfitAndLossSummary   AS PL 
		...
		INNER JOIN ( SELECT OrgSkey,Case @pLanguage When @ENLanguage THEN	Case @pGroup1Level	WHEN 'A' THEN	DO.[TerritoryDescEN]
				...
				LEFT OUTER JOIN [dbo].[FactGLBudgetingProcess] FB ON PL.[PostPeriodSkey] = FB.[PostPeriodSkey]--LEFT OUTER JOIN [dbo].[FactGLBudgetingProcessDetail] FB ON PL.[PostPeriodSkey] = FB.[PostPeriodSkey] --Add Budget Amt
WHERE        (PL.PostPeriodSkey = @pFiscalPeriod) 
			AND (FT.TemplateID = @pTemplate)
			AND PL.OrgSkey IN	( Select * from dbo.STRING_SPLIT(@pOrg, ','))
			AND (cast(FB.[BudgetRevision] as varchar(20)) = @pBudgetRevision or  FB.[BudgetRevision] is null)
			--and RemovedFlag = 'N'
...
GROUP BY	  
 ...
 ,ProfitandLossCTE_LY (Group1Level
					  ,Group2Level
					  ,Group3Level	
					  ,Group1LevelID
					  ,Group2LevelID
					  ,Group3LevelID,GroupID, TemplateID,[IsNetRevenues] ,[IsDirectLabor] ,[IsFringeBenefits]
					  ,[IsEBGE] ,[IsEBRE] ,[IsEBITDA] ,[IsManagementSalaries],[IsMarketingSalaries],[IsTrainingSalaries]
					  ,FiscalPeriodNumber, [PYActual])
			AS (Select Group1Level
					  ,Group2Level
					  ,Group3Level	
					  ,Group1LevelID
					  ,Group2LevelID
...
LEFT OUTER JOIN ProfitandLossCTE_LY as LYCTE ON  Template.TemplateID = LYCTE.TemplateID
													...
													AND Template.GroupID = LYCTE.GroupId
													AND Template.Group1level = LYCTE.Group1level
													AND Template.Group2level = LYCTE.Group2level
													AND Template.Group3level = LYCTE.Group3level
LEFT OUTER JOIN(
					Select   GroupID,Group1Level,Group2Level,Group3Level--OrganizationID	
				   ...
				   --,[PYActual] as [PYActual_Net_Revenues]--16
					FROM ProfitandLossCTE
					WHERE [IsNetRevenues] = 'Y')NRCTE  on  Template.Group1level = NRCTE.Group1level
													   AND Template.Group2level = NRCTE.Group2level
													   AND Template.Group3level = NRCTE.Group3level-- Template.OrganizationID = NRCTE.OrganizationID 
LEFT OUTER JOIN (	SELECT  Group1Level,Group2Level,Group3Level,
							[PYActual] as [PYActual_Net_Revenues]
							FROM ProfitandLossCTE_LY
							WHERE [IsNetRevenues] = 'Y')LYNRCTE on   Template.Group1level = LYNRCTE.Group1level
													             AND Template.Group2level = LYNRCTE.Group2level
													             AND Template.Group3level = LYNRCTE.Group3level--Template.OrgSkey = LYNRCTE.OrgSkey 
LEFT OUTER JOIN (
					Select GroupID,Group1Level,Group2Level,Group3Level --OrganizationID
				   ...
				   --,[PYActual] as [PYActual_Direct_Labor]
					...
					WHERE [IsDirectLabor] = 'Y'
							   )DLCTE on Template.Group1level = DLCTE.Group1level
...
LEFT OUTER JOIN (	SELECT  Group1Level,Group2Level,Group3Level,
							[PYActual] as [PYActual_Direct_Labor]
							FROM ProfitandLossCTE_LY
							WHERE [IsDirectLabor] = 'Y')LYDLCTE on   Template.Group1level = LYDLCTE.Group1level
													   AND Template.Group2level = LYDLCTE.Group2level
													   AND Template.Group3level = LYDLCTE.Group3level 
													   AND Template.GroupId in (94,95,96,100,106,108)
LEFT OUTER JOIN (
				     Select GroupID,Group1Level,Group2Level,Group3Level--OrganizationID
				  ...
				  -- ,[PYActual] as [PYActual_FringeBenefits]
					...
					WHERE [IsFringeBenefits] = 'Y'
							   )FBCTE on  Template.Group1level = FBCTE.Group1level
													   AND Template.Group2level = FBCTE.Group2level
													   AND Template.Group3level = FBCTE.Group3level  
													   AND Template.GroupId in (94,95,96,100,106,108)
LEFT OUTER JOIN (	SELECT Group1Level,Group2Level,Group3Level--OrgSkey
							,[PYActual] as [PYActual_FringeBenefits]
							FROM ProfitandLossCTE_LY
							WHERE [IsFringeBenefits] = 'Y')LYFBCTE on   Template.Group1level = LYFBCTE.Group1level
													   AND Template.Group2level = LYFBCTE.Group2level
													   AND Template.Group3level = LYFBCTE.Group3level  --Template.OrgSkey = LYFBCTE.OrgSkey 
														AND Template.GroupId in (94,95,96,100,106,108)
				   ...
				   --,SUM([PYActual]) AS [PYActual_Salaries]
					...
					WHERE [IsDirectLabor] = 'Y' OR [IsManagementSalaries] = 'Y'
						 OR [IsMarketingSalaries] = 'Y' OR [IsTrainingSalaries] = 'Y'
							GROUP BY  Group1Level,Group2Level,Group3Level--OrganizationID
													...
													AND Template.Group3level = SACTE.Group3level--Template.OrganizationID = SACTE.OrganizationID
																								--AND Template.GroupId in (94,95,96,100,106,108)
LEFT OUTER JOIN (	SELECT Group1Level,Group2Level,Group3Level 
							 ,SUM([PYActual]) AS [PYActual_Salaries]
							FROM ProfitandLossCTE_LY
							WHERE [IsDirectLabor] = 'Y' OR [IsManagementSalaries] = 'Y'
						 OR [IsMarketingSalaries] = 'Y' OR [IsTrainingSalaries] = 'Y'
							GROUP BY Group1Level,Group2Level,Group3Level  )LYSACTE on    Template.GroupId in (94,95,96,100,106,108)
													AND Template.Group1level = LYSACTE.Group1level
													AND Template.Group2level = LYSACTE.Group2level
													AND Template.Group3level = LYSACTE.Group3level--Template.OrgSkey = LYSACTE.OrgSkey 
																								--AND Template.GroupId in (94,95,96,100,106,108)
LEFT OUTER JOIN (
					Select  GroupID,Group1Level,Group2Level,Group3Level  --OrganizationID
...
LEFT OUTER JOIN (	SELECT   Group1Level,Group2Level,Group3Level 
							 ,[PYActual] as [PYActual_EBGE]
							FROM ProfitandLossCTE_LY
							WHERE [IsEBGE] = 'Y' )LYEBGECTE on  Template.GroupId in (94,95,96,100,106,108)
													AND Template.Group1level = LYEBGECTE.Group1level
													AND Template.Group2level = LYEBGECTE.Group2level
													AND Template.Group3level = LYEBGECTE.Group3level  
LEFT OUTER JOIN (
...
LEFT OUTER JOIN (	SELECT Group1Level,Group2Level,Group3Level 
							 ,[PYActual] as [PYActual_EBRE]
							FROM ProfitandLossCTE_LY
							WHERE [IsEBRE] = 'Y' )LYEBRECTE on  Template.GroupId	in (94,95,96,100,106,108)
													AND Template.Group1level = LYEBRECTE.Group1level
													AND Template.Group2level = LYEBRECTE.Group2level
													AND Template.Group3level = LYEBRECTE.Group3level  
LEFT OUTER JOIN (
					Select GroupID,Group1Level,Group2Level,Group3Level--OrganizationID
...
LEFT OUTER JOIN (	SELECT   Group1Level,Group2Level,Group3Level 
							 ,[PYActual] as [PYActual_EBITDA]
							FROM ProfitandLossCTE_LY
							WHERE [IsEBITDA] = 'Y' )LYEBITDACTE on   Template.GroupId  in (94,95,96,100,106,108) --EBITDACTE.GroupId
													AND Template.Group1level = LYEBITDACTE.Group1level
													AND Template.Group2level = LYEBITDACTE.Group2level
													AND Template.Group3level = LYEBITDACTE.Group3level
