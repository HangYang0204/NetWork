...
/****** Object:  StoredProcedure [dbo].[usp_ProfitandLoss_GraphsMarkup]    Script Date: 24/11/2016 11:00:57 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE Procedure [dbo].[usp_ProfitandLoss_GraphsMarkup] 
	@pCurrency varchar(5),
	@pFiscalPeriod int,
	@pLanguage int,
	@pOrg varchar(max),
...
BEGIN
					 ...
					 ,ISNULL(Case @pInCurrency WHEN @ConsCurrency THEN 
								(CASE CP.FiscalPeriodNumber WHEN 1   THEN SUM((PL.ConsP1FC2Amt) * FT.IncomeStatementActualSign)									  
											  ...
											  WHEN 8  THEN SUM(((PL.ConsP1FC2Amt + PL.ConsP2FC2Amt + PL.ConsP3FC2Amt + PL.ConsP4FC2Amt + PL.ConsP5FC2Amt ) * FT.IncomeStatementActualSign) 
													 + (( PL.ConsP6FC2Amt + PL.ConsP7FC2Amt + PL.ConsP8FC2Amt + PL.ConsP3FC2Amt + PL.ConsP4FC2Amt + PL.ConsP5FC2Amt) * FT.IncomeStatementBudgetSign))									  
											  WHEN 9  THEN SUM(((PL.ConsP1FC2Amt + PL.ConsP2FC2Amt) * FT.IncomeStatementActualSign) 
								 ...
								 WHEN @FuncCurrency THEN	 
										(CASE CP.FiscalPeriodNumber WHEN 1 THEN SUM((PL.FuncP1FC2Amt) * FT.IncomeStatementActualSign) 
												...
												WHEN 8 THEN SUM(((PL.FuncP1FC2Amt + PL.FuncP2FC2Amt + PL.FuncP3FC2Amt + PL.FuncP4FC2Amt + PL.FuncP5FC2Amt ) * FT.IncomeStatementActualSign) 
															+ (( PL.FuncP6FC2Amt + PL.FuncP7FC2Amt + PL.FuncP8FC2Amt + PL.FuncP3FC2Amt + PL.FuncP4FC2Amt + PL.FuncP5FC2Amt) * FT.IncomeStatementBudgetSign))  
												WHEN 9 THEN SUM(((PL.FuncP1FC2Amt + PL.FuncP2FC2Amt) * FT.IncomeStatementActualSign) 
		...
		WHERE     PL.PostPeriodSkey = (@pInFiscalPeriod)
					...
					AND (FT.IsNetRevenues = 'Y'  OR [IsDirectLabor] = 'Y' )
		...
		Select NR.[Level Code], NR.[Level Desc],NR.[Level Sort Order],NR.FiscalPeriodNumber
				,COALESCE(NR.[Current Month Amount]/ NULLIF(DL.[Current Month Amount],0),0)  as CM_Markup
				,COALESCE( NR.[Current Month Budget Amount]/ NULLIF(DL.[Current Month Budget Amount],0),0) as CM_BU_Markup
				,COALESCE( NR.[YTD Amount]/NULLIF(DL.[YTD Amount],0),0) as YRD_Markup
				,COALESCE( NR.[Current Month Forecast 1 Amount]/ NULLIF(DL.[Current Month Forecast 1 Amount],0),0.0)   as CM_F1_Markup
				,COALESCE(NR.[Current Month Forecast 2 Amount]/NULLIF(DL.[Current Month Forecast 2 Amount],0),0.0)  AS CM_F2_Markup
				,COALESCE(NR.[Current Month Forecast 3 Amount]/ NULLIF(DL.[Current Month Forecast 3 Amount],0),0.0) as CM_F3_Markup
				,COALESCE( NR.[YTD Budget Amount]/ NULLIF(DL.[YTD Budget Amount],0),0) as YRD_BU_Markup
				,COALESCE(NR.[Current Year Budget Amount]/NULLIF(DL.[Current Year Budget Amount],0),0.0) as CY_Bu_Markup
				,COALESCE(NR.[YTD Forecast 1 Amount]/ NULLIF(DL.[YTD Forecast 1 Amount],0),0.0) as YTD_F1_Markup
				,COALESCE(NR.[YTD Forecast 2 Amount]/NULLIF(DL.[YTD Forecast 2 Amount],0),0.0) as YTD_F2_Markup
				,COALESCE(NR.[YTD Forecast 3 Amount]/NULLIF(DL.[YTD Forecast 3 Amount],0),0.0) as YTD_F3_Markup
		FROM 
		(Select * FROM 	ProfitandLossCTE
			...
			INNER JOIN 
				(Select * from ProfitandLossCTE
					WHERE GroupID = -27)DL ON NR.[Level Code] = DL.[Level Code]
											
		ORDER BY 	 NR.[Level Sort Order]
